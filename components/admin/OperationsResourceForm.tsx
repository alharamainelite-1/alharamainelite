...
export function OperationsResourceForm({id,hostId,vehicleId,staffId,hosts,vehicles,staff}:{id:string;hostId:string|null;vehicleId:string|null;staffId:string|null;hosts:{id:string;name:string}[];vehicles:{id:string;vehicle_id:string}[];staff:{id:string;name:string}[]}){
 ...
 return <div className="grid gap-1">
  <select value={u} onChange={e=>setU(e.target.value)} className="!py-1 text-xs"><option value="">{ar?'لا يوجد موظف':'No staff'}</option>{staff.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select>
  <select value={h} onChange={e=>setH(e.target.value)} className="!py-1 text-xs"><option value="">{ar?'لا يوجد مضيف':'No host'}</option>{hosts.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select>
  <select value={v} onChange={e=>setV(e.target.value)} className="!py-1 text-xs"><option value="">{ar?'لا توجد مركبة':'No vehicle'}</option>{vehicles.map(x=><option key={x.id} value={x.id}>{x.vehicle_id}</option>)}</select>
  <button disabled={b} onClick={save} className="btn btn-outline !px-2 !py-1 text-xs">{b?(ar?"جارٍ الحفظ…":"Saving…"):(ar?"إسناد":"Assign")}</button>
 </div>
}