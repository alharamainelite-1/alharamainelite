...
type {t.role}='SUPER_ADMIN'|'ADMIN'|'OPERATIONS_MANAGER'|'OPERATIONS'|'SALES'|'FINANCE'|'HOST';
type User={id:string;email:string;full_name:string;phone:string;role:{t.role};created_at:string;email_confirmed:boolean;banned:boolean};
const roles:{t.role}[]=['ADMIN','OPERATIONS_MANAGER','OPERATIONS','SALES','FINANCE','HOST'];
const descriptions:Record<{t.role},string>={
...
export default function TeamManager(){
 ...
 const [form,setForm]=useState({full_name:'',email:'',phone:'',role:'SALES' as {t.role}});
 const [locale,setLocale]=useState<'en'|'ar'>('en'); const t=adminText[locale];
 ...
 async function update(userId:string,body:object){setBusy(true);setMessage('');const r=await fetch('/api/admin/team',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId,...body})});const j=await r.json();setMessage(r.ok?'{t.access} updated.':j.error||'Unable to update access.');if(r.ok)load();setBusy(false)}
