...
export default async function CityHub(){
  ...
  return <div>
    <section className="bg-forest text-white"><div className="container py-24 md:py-32"><div className="max-w-4xl"><div className="eyebrow">{labels.eyebrow}</div><h1 className="serif mt-5 text-5xl md:text-7xl">{labels.title}</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-white/75">{labels.intro}</p><Link href={localizedPath("/request-journey",locale)} className="btn mt-8 bg-gold text-forest">{labels.request}</Link></div></div></section>
