...
const image='https://images.pexels.com/photos/34744920/pexels-photo-34744920.jpeg?auto=compress&cs=tinysrgb&w=1800';

export default async function Jeddah(){
  const raw=(await cookies()).get('he_locale')?.value;
  const l=isLocale(raw)?raw:defaultLocale;
  const t=pageCopy[l].jeddah;
  const packagesPath=localizedPath('/packages',l);
  const requestPath=localizedPath('/request-journey',l);
  const pagePath=localizedPath('/jeddah',l);
  const schema={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[
    ...
    {'@type':'ListItem',position:2,name:'Jeddah',item:`${SITE_URL}${pagePath}`}
  ]};
  return <div>
