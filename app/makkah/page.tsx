...
const image='https://images.pexels.com/photos/32839113/pexels-photo-32839113.jpeg?auto=compress&cs=tinysrgb&w=2200';

export default async function Makkah(){
  const raw=(await cookies()).get('he_locale')?.value;
  const l=isLocale(raw)?raw:defaultLocale;
  const t=pageCopy[l].makkah;
  const packagesPath=localizedPath('/packages',l);
  const requestPath=localizedPath('/request-journey',l);
  const pagePath=localizedPath('/makkah',l);
  const schema={'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[
    ...
    {'@type':'ListItem',position:2,name:'Makkah',item:`${SITE_URL}${pagePath}`}
  ]};
  return <div>
