...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const cityPages = CITY_SEO.flatMap((city) => {
    const path = `/umrah-from-city/${city.slug}`;
    return (['en','so','ar'] as const).map((locale) => ({ url: `${SITE_URL}${localizedPath(path, locale)}`, lastModified, changeFrequency: 'monthly' as const, priority: 0.85 }));
  });
