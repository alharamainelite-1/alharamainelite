...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const cityPages = CITY_SEO.flatMap((city) => (['en','so','ar'] as const).map((locale) => ({ url: `${SITE_URL}${localizedPath(`/umrah-from-city/${city.slug}`, locale)}`, lastModified, changeFrequency: 'monthly' as const, priority: 0.85 })));
