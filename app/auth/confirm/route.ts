...
export async function GET(req:Request){
  ...
  const supabase=createServerClient(URL,KEY,{cookies:{getAll(){return store.getAll()},setAll(items){items.forEach(({name,value,options})=>store.set(name,value,options))}}});
