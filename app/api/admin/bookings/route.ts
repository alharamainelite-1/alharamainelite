...
const ALLOWED: Record<string,string[]> = {
  SALES: ['CONTACTED','DETAILS_PENDING','PAYMENT_PENDING','PAYMENT_RECEIVED','CANCELLED'],
...
export async function PATCH(req: Request) {
  ...
  if (!body?.bookingId || !body.status || !STATUS.includes(body.status as typeof STATUS[number])) {
  ...
  if (body.status === 'CONFIRMED' && before.payment_status !== 'RECEIVED') {
    return NextResponse.json({ error: 'A booking can only be confirmed after finance marks payment as received.' }, { status: 409 });
  }
