...
export default async function GroupsPage(){ const staff=await getCurrentStaff(); if(!staff)return null; const canManage=['SUPER_ADMIN','ADMIN','OPERATIONS_MANAGER'].includes(staff.profile.role); const locale=await getAdminLocale(); const t=adminText[locale];
  ...
  return <section className="pb-12">
    <div className="eyebrow">{locale==='ar'?'إدارة العمليات':'Operations manager'}</div><h1 className="serif mt-2 text-4xl text-forest">{t.page.groups}</h1><p className="mt-2 text-sm text-forest/55">{locale==='ar'?'تخطيط المجموعات الصغيرة المعتمدة وتعيين الموارد التشغيلية.':'Approved small-group planning and operational assignments.'}</p>
