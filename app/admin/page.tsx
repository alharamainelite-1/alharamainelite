...
export default async function AdminHome(){
 ...
 try{
  ...
  const [r,a,p,d,taskCount,pay,j]=await Promise.all([s.from("journey_requests").select("*",{count:"exact",head:true}),s.from("bookings").select("*",{count:"exact",head:true}).in("status",["CONFIRMED","PREPARING","ACTIVE"]),s.from("bookings").select("*",{count:"exact",head:true}).in("payment_status",["PAYMENT_INSTRUCTIONS_SENT","PENDING_VERIFICATION"]),s.from("bookings").select("*",{count:"exact",head:true}).eq("status","COMPLETED"),s.from("operations_tasks").select("*",{count:"exact",head:true}).in("status",["OVERDUE","DELAYED"]),s.from("payments").select("amount,status"),s.from("bookings").select("id,booking_id,status,payment_status,total_amount,guest_count,created_at,customers(full_name),packages(name)").order("created_at",{ascending:false}).limit(6)]);
