      ...
      <table className="w-full min-w-[980px] text-left text-sm">
        ...
        <tbody>
          ...
          rows.map((row:any)=><tr key={row.id} className="border-b border-forest/8 last:border-0">
            ...
            <td className="px-4 py-4">{(locale==='ar'?({NEW_REQUEST:'طلب جديد',CONTACTED:'تم التواصل',DETAILS_PENDING:'بانتظار التفاصيل',PAYMENT_PENDING:'بانتظار الدفع',PAYMENT_RECEIVED:'تم استلام الدفع',CONFIRMED:'مؤكد',PREPARING:'قيد التجهيز',ACTIVE:'نشطة',COMPLETED:'مكتملة',CANCELLED:'ملغاة'} as any)[row.status]||row.status:row.status}</td>
            {canViewFinancial&&<td className="px-4 py-4">{(locale==='ar'?({NOT_REQUESTED:'لم يُطلب',PENDING:'معلق',PENDING_VERIFICATION:'بانتظار التحقق',RECEIVED:'تم الاستلام',PARTIALLY_RECEIVED:'مستلم جزئيًا',REFUNDED:'مسترد',FAILED:'فشل'} as any)[row.payment_status]||row.payment_status:row.payment_status}</td>}
            <td className="px-4 py-4 text-xs text-forest/45">{new Date(row.created_at).toLocaleDateString(locale==='ar'?'ar-SA':'en-GB')}</td>
            <td className="px-4 py-4">{staff.profile.role==='FINANCE'?<span className="text-xs text-forest/55">locale==='ar'?'المالية: التحقق من الدفع فقط':'Finance: payment verification only'</span>:<BookingStatusForm bookingId={row.id} currentStatus={(locale==='ar'?({NEW_REQUEST:'طلب جديد',CONTACTED:'تم التواصل',DETAILS_PENDING:'بانتظار التفاصيل',PAYMENT_PENDING:'بانتظار الدفع',PAYMENT_RECEIVED:'تم استلام الدفع',CONFIRMED:'مؤكد',PREPARING:'قيد التجهيز',ACTIVE:'نشطة',COMPLETED:'مكتملة',CANCELLED:'ملغاة'} as any)[row.status]||row.status:row.status} paymentStatus={(locale==='ar'?({NOT_REQUESTED:'لم يُطلب',PENDING:'معلق',PENDING_VERIFICATION:'بانتظار التحقق',RECEIVED:'تم الاستلام',PARTIALLY_RECEIVED:'مستلم جزئيًا',REFUNDED:'مسترد',FAILED:'فشل'} as any)[row.payment_status]||row.payment_status:row.payment_status}/>}</td>
          </tr>)}
        </tbody>
      </table>
    </div>
