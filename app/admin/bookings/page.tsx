    ...
    <div className="card mt-6 overflow-x-auto">
      <table className="w-full min-w-[980px] text-left text-sm">
        <thead className="border-b border-forest/10 bg-[#faf8f2]"><tr>
          {['Booking ID','Customer','Package','Source','Guests',...(canViewFinancial?['Total']:[]),'Booking status',...(canViewFinancial?['Payment']:[]),'Created','Update'].map(h=><th key={h} className="px-4 py-4 font-semibold text-forest">{h}</th>)}
        </tr></thead>
        <tbody>
