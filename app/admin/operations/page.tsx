...
export default async function OperationsPage(){
 ...
 return <section className="pb-12">
  ...
  <div className="card mt-6 overflow-x-auto"><table className="w-full min-w-[1180px] text-left text-sm"><thead><tr>{['Task','Type','Date','Time','Group','Assigned staff','Host','Vehicle','Location','Status','Update'].map(h=><th key={h} className="px-4 py-4">{h}</th>)}</tr></thead>
   <tbody>{rows.length===0?<tr><td colSpan={11} className="p-12 text-center text-forest/45">{t.common.noTasks}</td></tr>:rows.map(r=><tr key={r.id}>
    ...
    <td className="px-4 py-4">{team.find((x:any)=>x.id===r.assigned_staff_id)?.name||'Unassigned'}</td><td className="px-4 py-4">{r.assigned_host||'Unassigned'}</td><td className="px-4 py-4">{r.assigned_vehicle||'Unassigned'}</td><td className="px-4 py-4">{r.location||'—'}</td><td className="px-4 py-4">{r.status||'—'}</td>
