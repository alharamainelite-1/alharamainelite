...
export default async function HostTasksPage(){
  ...
  return <section className="pb-12">
    ...
    <div className="mt-7 grid gap-4">{(!tasks||tasks.length===0)?<div className="card p-10 text-center text-forest/45">لا توجد مهام مسندة حتى الآن.</div>:tasks.map(task=><div key={task.id} className="card p-6">
      ...
      {task.status!=='COMPLETED'&&task.status!=='CANCELLED'&&<div className="mt-5"><HostTaskStatusButton id={task.id} nextStatus={task.status==='ASSIGNED'?'ACCEPTED':task.status==='ACCEPTED'?'IN_PROGRESS':'COMPLETED'} label={task.status==='ASSIGNED'?'قبول المهمة':task.status==='ACCEPTED'?'بدء المهمة':'وضع علامة مكتملة'}/></div>}
    </div>)}</div>
  </section>;
}
