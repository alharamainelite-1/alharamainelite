...
export default async function StaffMonitoringPage({searchParams}:{searchParams:Promise<{staff?:string}>}){
 ...
 return <section className="pb-12">
  <div className="flex flex-wrap items-end justify-between gap-4"><div><div className="eyebrow">{locale==='ar'?'رقابة الأداء':'Staff oversight'}</div><h1 className="serif mt-2 text-4xl text-forest">{locale==='ar'?'متابعة الموظفين':'Staff Monitoring'}</h1><p className="mt-2 text-sm text-forest/55">{locale==='ar'?'متابعة تشغيلية للمهام والمواعيد والإجراءات.':'Operational monitoring of tasks, SLA deadlines and recorded actions.'}</p></div><div className="rounded-xl bg-[#f7f3ea] px-4 py-3 text-xs text-forest/60">Last 30 days</div></div>
  {error&&<div className="mt-6 border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</div>}
  <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-6">{[['Staff',profiles.length],['Logins',activity.filter(x=>x.event_type==='LOGIN').length],['Recorded actions',audit.length],['Open tasks',allOpen.length],['Overdue SLA',overdue.length],['SLA on-time',completed.filter(x=>x.completed_at&&x.sla_due_at).length?slaRate+'%':'—']].map(([k,v])=><div className="card p-5" key={String(k)}><div className="text-xs uppercase tracking-[.16em] text-forest/40">{k}</div><div className="serif mt-2 text-3xl text-forest">{String(v)}</div></div>)}</div>
  {dueSoon.length>0&&<div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>{dueSoon.length} task(s) due within 60 minutes.</strong> Review the assignments below.</div>}
  {overdue.length>0&&<div className="mt-3 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-900"><strong>{overdue.length} open task(s) are past their SLA.</strong> These require operational follow-up.</div>}

  <div className="card mt-6 overflow-x-auto"><div className="p-5"><h2 className="serif text-2xl text-forest">Employee activity</h2><p className="mt-1 text-xs text-forest/45">Operational facts only; no private-device or personal browsing monitoring.</p></div><table className="w-full min-w-[1300px] text-left text-sm"><thead className="border-y border-forest/10 bg-[#faf8f2]"><tr>{['Employee','Role','Logins','Last login','Actions','Today','Open','SLA overdue','Due soon','Unstarted','Last activity'].map(h=><th key={h} className="px-4 py-4">{h}</th>)}</tr></thead><tbody>{profiles.map(p=>{const x=byId.get(p.id);const z=summary(x);return <tr key={p.id} className="border-b border-forest/8"><td className="px-4 py-4 font-semibold"><Link className="hover:text-gold" href={'/admin/staff-monitoring?staff='+p.id}>{p.full_name||'Unnamed'}</Link></td><td className="px-4 py-4">{p.role}</td><td className="px-4 py-4">{x?.logins||0}</td><td className="px-4 py-4 text-xs">{fmt(x?.lastLogin)}</td><td className="px-4 py-4">{x?.actions||0}</td><td className="px-4 py-4">{z.todayTasks.length}</td><td className="px-4 py-4">{z.open.length}</td><td className="px-4 py-4">{z.over.length}</td><td className="px-4 py-4">{z.soon.length}</td><td className="px-4 py-4">{z.unstarted.length}</td><td className="px-4 py-4 text-xs">{fmt(x?.lastActivity)}</td></tr>})}</tbody></table></div>
