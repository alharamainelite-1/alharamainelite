...
export default async function StaffMonitoringPage({searchParams}:{searchParams:Promise<{staff?:string}>}){
 ...
 return <section className="pb-12">
  ...
  <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-6">{[ar?['الموظفون',profiles.length]:['Staff',profiles.length],ar?['تسجيلات الدخول',activity.filter(x=>x.event_type==='LOGIN').length]:['Logins',activity.filter(x=>x.event_type==='LOGIN').length],ar?['الإجراءات المسجلة',audit.length]:['Recorded actions',audit.length],ar?['المهام المفتوحة',allOpen.length]:['Open tasks',allOpen.length],ar?['تجاوز SLA',overdue.length]:['Overdue SLA',overdue.length],ar?['الالتزام بالوقت',completed.filter(x=>x.completed_at&&x.sla_due_at).length?slaRate+'%':'—':['SLA on-time',completed.filter(x=>x.completed_at&&x.sla_due_at).length?slaRate+'%':'—']].map(([k,v])=><div className="card p-5" key={String(k)}><div className="text-xs uppercase tracking-[.16em] text-forest/40">{k}</div><div className="serif mt-2 text-3xl text-forest">{String(v)}</div></div>)}</div>
