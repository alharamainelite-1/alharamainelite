...
-- Security hardening: atomic partner payouts, application rate limiting,
-- least-privilege defaults, and transactional payout audit records.
...
begin
  ...
  if p_actor_id is null or v_partner.user_id <> p_actor_id then
    raise exception 'partner_actor_mismatch';
  end if;

  if v_partner.status not in ('PENDING', 'ACTIVE') then
  ...
  if v_balance < 500 then
    raise exception 'minimum_payout_balance';
  end if;
  ...
  if not found then
    raise exception 'payout_reservation_failed';
  end if;
  ...
  return query
  select v_payout.id, v_payout.amount, v_payout.status, v_payout.requested_at;
end;
$$;
...
-- Stop future public-schema objects from being automatically exposed to client roles.
...
-- Remove anonymous write capability from existing public tables; authenticated
-- access is retained because staff/admin RLS policies intentionally support
-- browser-side authenticated operations.
do $$
declare
  r record;
begin
