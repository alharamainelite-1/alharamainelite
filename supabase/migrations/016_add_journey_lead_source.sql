alter table public.journey_requests add column if not exists lead_source text not null default 'PUBLIC' check (lead_source in ('PUBLIC','WOMENS_UMRAH'));
alter table public.bookings add column if not exists lead_source text not null default 'PUBLIC' check (lead_source in ('PUBLIC','WOMENS_UMRAH'));
...
begin
  ...
  insert into public.journey_requests(reference,customer_id,package_id,guest_count,expected_travel_date,expected_period_start,expected_period_end,expected_period_label,estimated_total,currency,additional_notes,lead_source)
  values(v_reference,v_customer_id,v_package.id,p_guest_count,p_expected_travel_date,p_expected_period_start,p_expected_period_end,nullif(trim(p_expected_period_label),''),v_total,v_package.currency,nullif(trim(p_additional_notes),''),v_source) returning id into v_request_id;
  v_booking_id:='HE-'||to_char(current_date,'YYYY')||'-'||lpad(nextval('public.booking_number_seq')::text,5,'0');
  insert into public.bookings(booking_id,request_id,customer_id,package_id,guest_count,total_amount,currency,status,payment_status,expected_travel_date,expected_period_start,expected_period_end,notes,lead_source)
  values(v_booking_id,v_request_id,v_customer_id,v_package.id,p_guest_count,v_total,v_package.currency,'NEW_REQUEST','NOT_REQUESTED',p_expected_travel_date,p_expected_period_start,p_expected_period_end,nullif(trim(p_additional_notes),''),v_source);
...
revoke execute on function public.create_journey_request(text,text,text,text,text,text,text,integer,date,date,date,text,text,text) from authenticated;